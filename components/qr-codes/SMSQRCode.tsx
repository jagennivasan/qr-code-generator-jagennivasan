...
export default function SMSQRCode() {
  ...
  const data =
  SMSForm.phoneNumber && SMSForm.message
    ? `sms:${SMSForm.countryCode}${
        SMSForm.phoneNumber
      }?body=${encodeURIComponent(SMSForm.message)}`
    : "";
  useEffect(() => {
    if (qrCode.current && SMSForm.phoneNumber && SMSForm.countryCode && SMSForm.message) {
  ...
  return (
    <div className="flex flex-col md:flex-row md:justify-between w-full space-y-5 md:space-y-0 md:space-x-5 items-center">
      <div className="w-full md:w-2/3 lg:w-3/4 max-w-3xl px-4">
        <h1 className="text-2xl mb-2">SMS</h1>
        ...
        <small className=" mt-1">QR Code for Direct Messaging.</small>
        <QRCodeColors
        ...
        <div className="text-center md:flex-col justify-center items-center my-5">
          <button
            ...
            className={`px-3 py-2 bg-orange-500 text-white rounded-lg  transition my-3 mx-3 ${
              !isPresent || !SMSForm.message || !SMSForm.phoneNumber
